...
export default function PochonCarousel() {
    ...
    return (
        ...
        <div className="container mx-auto px-4 w-full py-5 lg:py-5">
            ...
            {/* Grille principale avec espace équilibré */}
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6 h-[600px] md:h-[600px]">
                {/* Colonne gauche : 2 petites images en haut + 1 medium en bas */}
                <div className="col-span-12 md:col-span-6 flex flex-col gap-4 h-full">

                    {/* Ligne du haut : deux petites images */}
                    <div className="flex gap-4 flex-1">
                        {data.filter((item) => item.layout === "small")
                            .map((item) => (
                                <Card key={item.id} onClick={() => handleImageClick(item.imageUrl, item.id)} className="relative flex-1 rounded-none overflow-hidden cursor-pointer group"
                ...
                {/* Colonne droite : image large avec moins d'espace */}
                <div className="col-span-12 md:col-span-6 h-full">

                    {/* Colonne droite : image large */}
