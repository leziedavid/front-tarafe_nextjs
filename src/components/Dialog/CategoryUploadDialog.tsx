...
interface CategoryUploadDialogProps {
    ...
    categoryId?: number;  // Si l'ID est passé, on va modifier une catégorie: OptionRealisation | null;
...
const CategoryUploadDialog: React.FC<CategoryUploadDialogProps> = ({ open, onOpenChange, categoryId,datas}) => {
    ...
    useEffect(() => {
        if (categoryId) {
            setIsEditing(true);
            // Ici, tu récupères les données de la catégorie pour la modifier
            // fetchDataCategoryById(categoryId); // Par exemple, tu pourrais avoir une fonction qui récupère la catégorie existante
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (!result.success) {
                toast.error(result.error.errors[0].message); // Afficher la première erreur de validation
                return;
            }
            ...
            const resultApi = await saveCategory(token, categoryNames, categoryId);  // Le categoryId est optionnel

            if (resultApi.statusCode === 200) {
                toast.success(isEditing ? "Catégorie mise à jour avec succès !" : "Catégories ajoutées avec succès !");
                // resetForm();  // Réinitialiser les champs après la soumission
